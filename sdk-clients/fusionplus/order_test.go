...
func TestCreateFusionPlusOrderData(t *testing.T) {
	tests := []struct {
		name        string
		chainId     uint64
		privateKey  string
		orderParams OrderParams
		quoteParams QuoterControllerGetQuoteParamsFixed
	...
	}{
		{
			...
			quoteParams: QuoterControllerGetQuoteParamsFixed{
				...
				Amount:          "1000000",
				WalletAddress:   "0x3847f83b648fce07934b6841254b6acaeeda6e67",
				EnableEstimate:  true,
				Fee:             nil,
				IsPermit2:       false,
			},
			orderParams: OrderParams{
				HashLock: &HashLock{
					Value: "0xa9a1513afadecbb46f7b05181f8edb4b99b176ccf5b4312eca82d76fd4f2a0d9",
				},
				SecretHashes:      []string{"0xa9a1513afadecbb46f7b05181f8edb4b99b176ccf5b4312eca82d76fd4f2a0d9"},
			...
			quote: GetQuoteOutputFixed{
				...
				DstSafetyDeposit: "2929706640000",
				DstTokenAmount:   "",
				Presets: QuotePresets{
					Fast: Preset{
						StartAmount:        "807078",
						SecretsCount:       1,
						CostInDstToken:     "169881",
						AuctionDuration:    180,     // Converted from "180" (string) to 180 (float32)
						StartAuctionIn:     17,      // Converted from "17" (string) to 17 (float32)
						InitialRateBump:    2968281, // Assuming this is a valid float32 value
						AuctionStartAmount: "976958",
						AuctionEndAmount:   "753345",
						Points: []AuctionPoint{
							{
								Delay:       180,     // Converted from 180 (integer) to 180.0 (float32)
								Coefficient: 2255022, // Assuming this is a valid float32 value
							},
						},
						GasCost: GasCostConfig{
							GasPriceEstimate: "79",
							GasBumpEstimate:  2255022,
						},
					...
					Medium: Preset{
						...
						SecretsCount:       1,
						CostInDstToken:     "169881",
						AuctionDuration:    360,     // Converted from "360" (string) to 360 (float32)
						StartAuctionIn:     17,      // Converted from "17" (string) to 17 (float32)
						InitialRateBump:    2968281, // Assuming this is a valid float32 value
						AuctionStartAmount: "976958",
						AuctionEndAmount:   "753345",
						Points: []AuctionPoint{
							{
								Delay:       360,     // Converted from 360 (integer) to 360.0 (float32)
								Coefficient: 2255022, // Assuming this is a valid float32 value
							},
						},
						GasCost: GasCostConfig{
					...
					Slow: Preset{
						...
						SecretsCount:       1,
						CostInDstToken:     "169881",
						AuctionDuration:    600,     // Converted from "600" (string) to 600 (float32)
						StartAuctionIn:     17,      // Converted from "17" (string) to 17 (float32)
						InitialRateBump:    2968281, // Assuming this is a valid float32 value
						AuctionStartAmount: "976958",
						AuctionEndAmount:   "753345",
						Points: []AuctionPoint{
							{
								Delay:       600,     // Converted from 600 (integer) to 600.0 (float32)
								Coefficient: 2255022, // Assuming this is a valid float32 value
							},
						},
						GasCost: GasCostConfig{
				...
				Prices: PairCurrency{
					Usd: TokenPair{
						SrcToken: "0.9994076359882553",
						DstToken: "0.9979086311883734",
					},
				},
				QuoteId:           "62c67105-1ab7-4bfb-a4e1-7f650e20b832",
				...
				TimeLocks: TimeLocks{
					SrcWithdrawal:         60,
					SrcPublicWithdrawal:   420,
					SrcCancellation:       576,
					SrcPublicCancellation: 696,
					DstWithdrawal:         60,
					DstPublicWithdrawal:   360,
					DstCancellation:       480,
				},
				Volume: PairCurrency{
					Usd: TokenPair{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			fusionPlusOrder, err := CreateFusionPlusOrderData(tc.quoteParams, &quote, tc.orderParams, wallet, int(tc.chainId))
			require.NoError(t, err)
			...
			assert.Equal(t, expectedOrdrebookOrder, *fusionPlusOrder.LimitOrder)
			assert.Equal(t, expectedPreparedOrder, *fusionPlusOrder)
