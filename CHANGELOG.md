...
### Fixed
- **Funds-safety: cross-chain orders with a native destination token were signed with the WRONG taker asset.** When `DstTokenAddress` was the native-token sentinel, the wrapper was resolved on the **source** chain, baking e.g. Arbitrum's WETH address into the taker asset and escrow `DstToken` of an order settling on Base. The wrapper is now resolved on the destination chain, and chains without a registered wrapper fail loudly.
