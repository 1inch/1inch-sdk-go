...
- **User-Agent header**: API requests now report the actual SDK release version from `internal/version.Version`; the header was pinned to `v3.0.0`. The release workflow writes each new release's version into the constant and commits before tagging, so the header always matches the release tag.
