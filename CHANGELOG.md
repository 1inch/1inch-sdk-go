...
- **User-Agent header**: API requests now report the actual SDK version from the binary's build info; the header was pinned to `v3.0.0`. The resolved module version is reported verbatim whenever it is valid semver (release tags, prerelease tags, pseudo-versions), replace directives report the replacement's version, and only non-versions (a local path replacement or an in-repo build) report `unknown`.
