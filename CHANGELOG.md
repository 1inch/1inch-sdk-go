...
- Characterization test suite for the codegen pipeline (`codegen/codegen_test.go`): asserts that the pipeline reproduces every committed `*_types.gen.go` file byte-for-byte from the committed specs, that the specs are a fixed point of the in-place transforms, and documents each transform's behavior via synthetic fixtures. This makes the pipeline safe to restructure.
...
### Fixed
- **Funds-safety: `fusionplus.DecodeEscrowExtension` corrupted escrow terms.** Decoded output swapped the source and destination safety deposits, rendered deposits in hex where re-encoding parses decimal, and rendered the hashlock in decimal where re-encoding parses hex — so decode-then-re-encode silently changed the secret hash and deposits. All three are fixed and a lossless encode→decode round-trip test pins the behavior.
- **Funds-safety: `EscrowExtension.ConvertToOrderbookExtension` mutated the receiver**, double-appending the escrow extra data on a second call and producing a corrupted extension whose salt no longer matches. It is now pure and idempotent.
...
### Breaking Changes
- **Generated type names are now intent-based instead of mirroring upstream controller ids** (`fusionplus.QuoteParams` instead of `QuoterControllerGetQuoteParams`, `fusionplus.Quote` instead of `GetQuoteOutput`, `txbroadcast.BroadcastPublicTransactionJSONRequestBody` instead of `TxProcessorApiControllerBroadcastTransactionJSONRequestBody`, …). All 55 renamed types and 14 renamed constants keep their old names as `// Deprecated:` aliases (identical types — existing code compiles unchanged), except struct literals that named the embedded query field of the four `orderbook` wrapper params; see BREAKING_CHANGES.md.
...
- **Raw generated types now carry the spec corrections previously exclusive to the `*Fixed` types**: `QuoteId` string, `Amount` string, `Fee int`, `IsPermit2` bool, string USD prices, `Points` slice, `ExclusiveResolver` string (fusion/fusionplus). See BREAKING_CHANGES.md for the full list.
...
- **v5 public-API surface cleanup** across all client packages: intent-based renames (e.g. `history.Item`→`HistoryEvent`, `orderbook.Decode`→`DecodeExtension`, `fusionplus.GetOrderByOrderHash`→`GetOrderFillsByHash`, `balances` `…List`-suffix drops, `tokens.WhitelistedTokens`→`GetWhitelistedTokens`, portfolio `ProfitLoss`→`ProfitAndLoss`, `fusion` clean `Preset`/`AuctionPoint`/… names, `constants.NetworkEnum` deprecated in favor of `int` chain ids), plus removal of dead unused public types. **Almost all renames keep the old name as a `// Deprecated:` alias or forwarding method, so existing code compiles unchanged.** The few unavoidable hard breaks (no alias possible) are: `common.RequestPayload.U`→`.Path`, `fusionplus.Order.EscExtension`→`.EscrowExtension`, `traces.NewConfiguration` now takes `ConfigurationParams`, `constants.GetWrappedToken`/`ChainToWrapper` keyed by `int`, and the removed dead types. See BREAKING_CHANGES.md → "API Surface Cleanup (v5)" and MIGRATION.md for the full list.
