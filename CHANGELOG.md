...
- **Type generator upgraded from deprecated `deepmap/oapi-codegen` v1.16.2 to `oapi-codegen/oapi-codegen` v2.8.0.** Generated types are unchanged except: bare currency constants in `spotprices` are now prefixed with their type name (`spotprices.USD` → `spotprices.GetPricesForRequestedTokensParamsCurrencyUSD`), and the unused `web3.ApiKeyAuthScopes` constant is gone. Additions: enums gain a `Valid()` method, response types gain doc comments, and `traces` gains typed union helpers.
